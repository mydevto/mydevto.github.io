...
// localStorage.setItem("model",`${userModel.value}`)
// loadFBXmodels(localStorage.getItem("model"))

previewer.setAttribute("z-index", "1000");
...
function loadFBXmodels(path) {
  fbxLoader.load(
    ...
    (xhr) => {
      ...
      load_text.innerHTML = `${(xhr.loaded / xhr.total) * 100}% loaded`;
    },
    (error) => {
      console.log(error);
    }
